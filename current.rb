...
def output_page(url, searchClass)
  doc = Nokogiri::HTML(open(url))
  doc.css('.scoringtable').each do |div|
    if div.css(searchClass).first.inner_html.include? "Gentlemen's Singles"
...
puts "---- Live ----"
output_page("http://wimbledon.com/en_GB/scores/index2.html", '.eventinfo')
puts "---- Completed ----"
output_page("http://www.wimbledon.com/en_GB/scores/results/#{find_day}.html", '.courtname')
